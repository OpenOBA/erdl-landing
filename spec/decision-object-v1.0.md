...
> **注**：`ROLLBACK`、`QUARANTINE`、`NOTIFY` 和 `ESCALATE` 的 Ring 1/2 级实现为 Pro 功能。Free 层（Ring 3）支持 `ALLOW`、`CORRECT`、`REQUEST_HUMAN`、`DENY`。`EMERGENCY_HALT` 仅在企业版 Ring 0 中可用。`BLOCK` 在本规范中等价于 `DENY`——两者语义相同，`DENY` 为本规范的统一术语。
...
| **2** | 运营环 | NOTIFY, 速率限制（Pro） | 运营/业务团队 | 部门主管 |
| **3** | 执行环 | ALLOW, CORRECT, REQUEST_HUMAN（Free） | 开发/个人 | 个人开发者 |
...
2. 加载向量集 → 运行每条向量 → 比较输出
3. 所有 23 条向量的 expected 与 actual 逐字节一致 → 声明兼容
