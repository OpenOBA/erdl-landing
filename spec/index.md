...
| `DENY` | Ring 0 | 直接拒绝（`BLOCK` 已废弃，等价于 `DENY`） |
...
Guard 的 then 仅支持 Ring 0-2 的动作：`DENY`、`EMERGENCY_HALT`、`QUARANTINE`、`ROLLBACK`、`REQUEST_HUMAN`、`ESCALATE`、`CORRECT`。（`BLOCK` 为 `DENY` 的废弃别名。）
...
5. 标记 `override: true` 的规则可以覆盖之前匹配的结果（仅覆盖 `DENY` → `ALLOW` 方向，不允许覆盖到更不安全的状态）
